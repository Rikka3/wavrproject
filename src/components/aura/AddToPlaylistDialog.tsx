  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ border: '1px solid rgba(255,255,255,0.1)', maxHeight: '70vh' }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <p className="text-[11px] font-extrabold uppercase tracking-widest text-white">ADD TO PLAYLIST</p>
          <button className="text-white/30 hover:text-white" onClick={() => { setShowAddToPlaylist(false); setAddToPlaylistSongId(null); setShowNew(false); }}>
            <X size={16} />
          </button>
        </div>
        ...
        {showNew ? (
          <div className="p-4">
            <input
              ...
              autoFocus
              className="w-full px-3 py-2.5 bg-white/5 text-white text-[11px] uppercase tracking-wide placeholder:text-white/15 outline-none mb-3"
              style={{ border: '1px solid rgba(255,255,255,0.1)' }}
            />
            <div className="flex gap-2">
              <button
                className="flex-1 py-2 text-[10px] uppercase font-bold tracking-wider text-white/40 hover:text-white/60"
                style={{ border: '1px solid rgba(255,255,255,0.1)' }}
              ...
              <button
                className="flex-1 py-2 text-[10px] uppercase font-bold tracking-wider text-white disabled:opacity-30"
          ...
          <div className="max-h-60 overflow-y-auto custom-scroll">
            ...
            >
              <div className="w-8 h-8 flex items-center justify-center" style={{ border: '1px dashed rgba(255,255,255,0.15)' }}>
                <Plus size={12} className="text-white/30" />
              </div>
              <span className="text-[11px] font-bold uppercase tracking-wide text-white/40">CREATE NEW PLAYLIST</span>
            </button>

            {/* Existing playlists */}
            {playlists.length === 0 ? (
              <div className="py-8 flex flex-col items-center">
                <ListMusic size={20} className="text-white/6 mb-2" strokeWidth={1} />
                <p className="text-[9px] text-white/15 uppercase tracking-widest">NO PLAYLISTS YET</p>
              </div>
            ) : (
              playlists.map(pl => (
                <button
                  key={pl.id}
                  className="w-full flex items-center gap-3 px-4 py-3 hover:bg-white/[0.03] transition-colors disabled:opacity-50"
                  style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}
                  onClick={() => handleAdd(pl.id)}
                  disabled={addingTo !== null}
                >
                  <div className="w-8 h-8 flex items-center justify-center" style={{ border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(0,0,0,0.3)' }}>
                    {addingTo === pl.id ? (
                      <div className="w-3 h-3 border border-white/40 border-t-transparent rounded-full animate-spin" />
                    ) : (
                      <ListMusic size={11} className="text-white/20" strokeWidth={1.5} />
                    )}
                  </div>
