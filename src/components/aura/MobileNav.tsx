...
export default function MobileNav() {
  ...
  return (
    <nav
      className="md:hidden fixed bottom-0 left-0 right-0 glass-bar"
      style={{
        zIndex: 10,
        position: 'fixed',
    ...
    >
      <div className="flex items-center justify-around px-2 py-2.5">
        <div className="flex items-center gap-1 absolute left-3">
          <Disc3 size={12} className="text-foreground/55" strokeWidth={2} />
          <span className="text-[8px] font-extrabold uppercase tracking-widest text-foreground/45">WAVE</span>
        </div>
        {tabs.map(tab => {
          ...
          return (
            ...
            >
              <tab.icon size={17} strokeWidth={active ? 2.5 : 1.8} />
              <span className={`text-[8px] font-bold uppercase tracking-widest ${active ? 'text-foreground' : 'text-foreground/45'}`}>{tab.label}</span>
